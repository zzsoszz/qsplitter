define(['jquery',"./jquery.debounce","jquery/qsplitter/1.0.4/jquery.qsplitter.css"], 
	...
	(function($)
	{
			...
			function QSplitter(target)
			{
				...
				this.start=function(event)
				{
						...
						if($(event.target).closest(this.splitbarelement).length>0)
						{
						    //绑定事件，记录鼠标起始位置
							$(this.element).on('mousemove.qsplitter',$.throttle($.proxy(this.draging,this),50));//节流
				...
				this.draging=function(event)
				{
					...
					if($(event.target).closest(this.element).length>0)
					{
						if(this.options.direction=="h") //横向移动分割条
						{
							...
							//$.throttle(this.spliting(distance),3000,this);
							this.spliting(distance);
							//_.throttle(this.spliting(distance), 300000);
						}else{
